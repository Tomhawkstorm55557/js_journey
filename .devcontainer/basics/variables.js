/*
there are thre types of variables in js let const var.....
const cant be changed 
var is the older 
var vs const 
The difference between let and var is in the scope of the variables they create: Variables declared by let are only available inside the block where they're defined. Variables declared by var are available throughout the function in which they're declared.
*/
let name = "vinayak";
/// when u pass without assigning any value it show un defined
let billo

const relation = " friends"// we cant change const value 
var id = "25678"
//// prit all these using
console.table([name,relation,id,billo]);
